...
内置 **3000 条日语词条**（N2 1000 + N1 2000；日文表记 / 假名注音 / 中文释义 / 日文例句 / 例句翻译），
...
- 首次启动时应用会把 `app/src/main/assets/jlpt_n2_words.json`（1000 条）与 `jlpt_n1_words.json`（2000 条）写入 Room 数据库；老版本升级时按 `jlpt='N1'` 计数增量补齐，既有学习进度不受影响。
...
- **输入行防遮挡**：内置键盘弹出时输入行抬升至键盘（含候选栏）正上方（`contentOverflowPx` 随键盘实测高度订阅更新），键入内容始终可见；候选栏位于输入行与按键之间。
- **滚动目标修正**：`lastMessageIndex` 统一计算最后一条消息的 item index（考虑「加载更早」哨兵占 index 0），自动滚动/回底按钮/流式钉底共用，修复「最新回复落到折叠区（n-1）」；`ChatHistoryWindow.sync` 在 id 不变时也用最新实例刷新内容（助手回复完成写库后必现）。
- **交互精简**：移除「回到顶部」↑ 按钮，仅保留「回到底部/最新」↓（按需显示）。
